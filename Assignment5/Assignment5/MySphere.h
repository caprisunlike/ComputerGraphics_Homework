...
GLuint MySphere::Init(int la_slice, int lo_slice, vec4 color)
{
	...
	for(int i=0; i<la_slice; i++)
	{
		...
		for(int j=0; j<lo_slice; j++)
		{
			...
			vec2 ta(j / float(lo_slice), 1 - i / float(la_slice));
			vec2 tb(j / float(lo_slice), 1 - (i+1) / float(la_slice));
			vec2 tc((j+1) / float(lo_slice), 1 - (i+1) / float(la_slice));
			vec2 td((j+1) / float(lo_slice), 1 - i / float(la_slice));
